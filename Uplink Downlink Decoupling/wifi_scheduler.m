...
%%
    ...
    ind_attach = cell2mat(param.attach(ID+1));
    if param.policy == 1
        tmp = [ue(ind_attach).attach_ul] > 0;
        ind_ul_nodes =[0, ind_attach(tmp)];
    else
        ind_ul_nodes = [0, ind_attach];
    end
    
    % Finding nodes which have data to send (contending nodes)
    contending_UE = [];
    if ~isempty(wifi(ID).tcp_data_dl)
        contending_UE = [contending_UE,0];
    end
    for i = ind_ul_nodes(2:end)
    ...
    % If Node0 = 0, then Wi-Fi AP has access to the channel    
    if Node0 == 0 
        %tmp = [wifi(ID).tcp_data_dl.destination];
        %UE0 = tmp(randi(length(tmp)));
        UE0 = [wifi(ID).tcp_data_dl(1).destination];
        rss = snr_wifi(wifi(ID),ue(UE0),1); % 1 is for downlink
        ...
        rss = snr_wifi(wifi(ID),ue(Node0),0); % 0 for uplink
        rate = snr_2_rate_wifi(wifi(ID),rss);
        if rate == 0
            return;
        end
        ...
        %for j = 1:ceil(data_to_send/param.data_pckt_size)
