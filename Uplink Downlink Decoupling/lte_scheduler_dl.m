...
%%
    ...
    if ~mod(param.t,bs.CQImeasure_period)
        for i = ind_attach
            ue(i).snr_dl = snr_macro(bs,ue(i),flag);
        end
    end
...
%                         [~,TBS] = snr_2_rate_macro(ue(u_j).snr_dl,alloc_PRB(u_j));
%                         alloc_TBS(u_j) = TBS;
...
%% Resource Allocation (Round robin scheduler)
    ...
    while prb_avail > 0
        ...
        if sum([bs.tcp_data_dl.destination] == u_i)            
            ...
            while u_i_satisfied == 0 && prb_avail > 0
                ...
                prb_avail = bs.nPRBs - sum(alloc_PRB);   
                [~,TBS] = snr_2_rate_macro(ue(u_i).snr_dl,alloc_PRB(u_i));
                alloc_TBS(u_i) = TBS;
    ...
    for i = ind_attach        
        p(i) = 0;
        if mod(param.t,bs.CQImeasure_period)
            current_snr = snr_macro(bs,ue(i),flag);
